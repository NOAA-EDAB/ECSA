...
* The environment within the stock area for summer flounder has changed in recent decades as indicated by increasing temperature and salinity. These changes may indicate a shift in ecosystem function and productivity. 
...
* Summer flounder diet has changed in recent decades, which may be driven by ecosystem effects on the distribution and availability of forage species. 

* Occupancy habitat models for summer flounder suggest the size of the summer flounder habitat is increasing, which may create a mismatch between the areas used by the species and the stock definition area.
...
<!--chapter:end:ECSA_summer-flounder.rmd-->
...
An optimal interpolation procedure was used to estimate NE Shelf surface and bottom temperatures for two seasonal time frames (see [methods](#sec:methodstempsalin)). The temperature estimates were standardized to April 3 and October 11 for spring and fall over the period 1968-2017. Spring surface and bottom temperature within the spring {{COMMON_NAME}} stock areas are shown in the upper and lower left figures. Fall surface and bottom temperature within the fall {{COMMON_NAME}} stock areas are shown in the upper and lower right figures. Error bars show &plusmn;1 SD of the observations.

### Figure

```{r ocean temp, echo = F, fig.align='center', warning=F, fig.cap = "Fall and spring surface and bottom temperature within {{COMMON_NAME}} stock areas.  Error bars show &plusmn;1 SD of the observations."}
...
DT::datatable(all_temp, rownames = FALSE,filter = list(position = 'top', clear = FALSE),
          extensions = 'Scroller',
          options = list(search = list(regex = TRUE),
  deferRender = TRUE,
  scrollY = 200,
...
DT::datatable(all_sal, rownames = FALSE,filter = list(position = 'top', clear = FALSE),
          extensions = 'Scroller',
          options = list(search = list(regex = TRUE),
  deferRender = TRUE,
  scrollY = 200,
...
### Figures
```{r chl fig, echo = F, fig.align="center", fig.height=3, warning=F, fig.cap = "Chlorophyll concentrations in the spring and fall {{COMMON_NAME}}  stock areas. Error bars show &plusmn;1 SD of the observations."}
...
DT::datatable(chl, rownames = FALSE,filter = list(position = 'top', clear = FALSE),
          extensions = 'Scroller',
          options = list(search = list(regex = TRUE),
  deferRender = TRUE,
  scrollY = 200,
...
### Data
```{r zoo data, echo = F}
DT::datatable(zoo, rownames = FALSE,filter = list(position = 'top', clear = FALSE),
          extensions = 'Scroller',
          options = list(search = list(regex = TRUE),
  deferRender = TRUE,
  scrollY = 200,
...
Trends in diet composition may indicate changes in the habitat for generalist predators, and may have implications for growth and productivity at the population level if the overall energy content of the diet changes. Food habits collected aboard NEFSC bottom trawl surveys in the {{COMMON_NAME}} seasonal survey strata were used to estimate weighted diet composition over time (see [methods](#sec:methodsdiet)), as well as length composition of sampled fish and sample size. Here we show seasonal weighted diet composition for major prey (averaging over 10 percent of diet across years). Food habits data for {{COMMON_NAME}} have been collected from 1977-present [@smith_trophic_2010].


```{r dietdat, eval=FALSE, echo = FALSE}
#summer flounder stomachs are collected XXX size strata. Over XX summer flounder stomachs have been collected to date from over XXX survey tows. Predator length ranged in size from X to X cm.
#[add: number+hauls sampled and length comp of sampled summer flounder in each time block below]
```
...
### Figures
```{r diet, eval=T, echo = FALSE, fig.align='center',warning=F, message=F,fig.cap = "Seasonal weighted diet composition for major {{COMMON_NAME}} (1977-2016)."}
  
df_diet <- get_diet(as.numeric(svspp)) #hidden output somewhere need to fix, function still broken dont use
...
### Data
```{r diet data, echo = F}
DT::datatable(diet, rownames = FALSE,filter = list(position = 'top', clear = FALSE),
          extensions = 'Scroller',
          options = list(search = list(regex = TRUE),
  deferRender = TRUE,
  scrollY = 200,
...
### Figures
```{r trawl-habitat, echo = FALSE, fig.align='center', warning=F, fig.height=3,  fig.cap = "Probability of occurrence of {{COMMON_NAME}} in NEFSC bottom trawl tows. Error bars represent the 95% confidence intervals."}


clean_survdat <- survdat %>%
  filter(STRATUM %in% c(strata_fall, strata_spring),
         YEAR > 1976) %>% 
  select(CRUISE6, YEAR, SEASON, STRATUM, STATION, LAT, LON, EST_TOWDATE) %>% 
  distinct(.keep_all = TRUE)

stock_survdat <- survdat %>% 
  filter(STRATUM %in% c(strata_fall, strata_spring),
         SVSPP == svspp,
         YEAR > 1976) %>% 
  group_by(CRUISE6, YEAR, SEASON, STRATUM, STATION, LAT, LON,
           EST_TOWDATE) %>% 
  summarize(BIOMASS = mean(BIOMASS)) %>% 
  right_join(clean_survdat, 
             by = c("CRUISE6", "YEAR", "SEASON", "STRATUM", "STATION", "LAT", "LON", "EST_TOWDATE")) %>% 
  mutate(pres = case_when(BIOMASS > 0 ~ TRUE,
                          BIOMASS == 0 ~ FALSE,
                          is.na(BIOMASS) ~ FALSE))


# df_survdat_spring <- #survdat %>%
#   df_survdat %>%
#   dplyr::filter(YEAR > 1976,
#                 # SVSPP == svspp,
#                 SEASON == "SPRING",
...
#                 STRATUM %in% strata_fall)
# 
# df_survdat2use <- rbind(df_survdat_spring,
#                         df_survdat_fall) %>%
#   dplyr::mutate(pres = BIOMASS > 0)


df_prob_occ <- stock_survdat %>% #df_survdat2use %>%
  dplyr::group_by(SEASON, YEAR) %>%
  dplyr::do(calc_p(df = .)) %>% 
  ungroup() %>% 
  mutate(SEASON = tolower(SEASON),
...
### Data
```{r prob data, echo = F}
DT::datatable(df_prob_occ, rownames = FALSE,
...
```{r prob data2, echo = F}
occ <- rbind(prob, hab)
DT::datatable(occ, rownames = FALSE,
              filter = list(position = 'top', clear = FALSE),
          extensions = 'Scroller',
          options = list(search = list(regex = TRUE),
  deferRender = TRUE,
...
<!--chapter:end:generic_template.rmd-->
...
The goal of ecsa is to ...

## Installation

You can install the released version of ecsa from [CRAN](https://CRAN.R-project.org) with:

``` r
install.packages("ecsa")
```
...
```{r my-table, echo=F, message=FALSE, warnings=FALSE}
df <- data.frame(Text = c("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut porttitor hendrerit diam.","Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut porttitor hendrerit diam."),
                    URL = c("www.alongurlcanbefoundhere.com","www.short.com"))
...
knitr::kable(
  ...
  kableExtra::column_spec(2, "5cm")
```

<!--chapter:end:table-test.Rmd-->
