...
An optimal interpolation procedure was used to estimate NE Shelf surface and bottom temperatures for two seasonal time frames (see [methods](#sec:methodstempsalin)). The temperature estimates were standardized to April 3 and October 11 for spring and fall over the period 1968-2017. Spring surface and bottom temperature within the spring summer flounder stock areas are shown in the upper and lower left figures. Fall surface and bottom temperature within the fall summer flounder stock areas are shown in the upper and lower right figures.
...
all_temp <- all_temp %>% mutate(Var, row = plyr::mapvalues(Var,
                                                   from = unique(Var),
                                                             to = c(1,1,2,2))) %>%
  group_by(row) %>%
...
ggplot(data = all_temp, aes(x = Time, y = Value)) +
...
datatable(all_temp, rownames = FALSE)

```

## Surface and bottom salinity {.tabset .tabset-fade .tabset-pills}

An optimal interpolation procedure was used to estimate NE Shelf surface and bottom salinity for two seasonal time frames (see [methods](#sec:methodstempsalin)). Though collected with temperature data, reliable instrumentation limits this time series to 1992-2017. The salinity estimates were standardized to April 3 and October 11 for spring and fall. Spring surface and bottom salinity within the spring summer flounder stock areas are shown in the upper and lower left figures. Fall surface and bottom salinity within the fall summer flounder stock areas are shown in the upper and lower right figures. 

Similar to fall temperature, in recent years bottom and surface salinity are at or near historical highs.
...
ggplot(data = all_sal, aes(x = Time, y = Value)) +
  ylab("Salinity (PSU)") +
  xlab("Year") +
  xlim(xmin, NA) +
  geom_line() +
...
datatable(all_sal, rownames = FALSE)

```

## Chlorophyll concentration {.tabset .tabset-fade .tabset-pills}

The concentration of chlorophyll was measured with a suite of satellite sensors and merged into a single dataset (see [methods](#sec:methodschl)). Chlorophyll concentrations in the spring and fall summer flounder stock areas are shown in the left and right figures. The time series of spring chlorophyll concentration may have impacts on recruitment and growth of the stock, while fall chlorophyll values may affect overwintering nutrition. 
...
Probability of occurrence has been relatively high in recent years in both seasons. 

### Figures
```{r trawl-habitat, echo = FALSE, fig.align='center', fig.height=3, warning=F}
...
# Make plot
proc_occ_plt <- ggplot(df_prob_occ, 
       aes(x = YEAR, y = p)) +
  geom_point()+
...
The occupancy probability has increased in both stock areas (see upper plots) and suggest similar trends to the GLM model results above for the overlapping period. The area of the Northeast Shelf with an estimated occurrence probability of 0.5 or greater has increased in both seasons. In spring, the occupancy area have increased from approximately 50,000 km$^2$ to 80,000 km$^2$, noting a decrease in 2017 (lower left plot). The fall habitat has increased from approximately 40,000 km$^2$ to 60,000 km$^2$ with no indication of the trend abating (lower right plot).
...
```{r prob data2, echo = F}
occ <- rbind(prob, hab)
datatable(occ, rownames = FALSE)
```

## Diet composition {.tabset .tabset-fade .tabset-pills}

Food habits collected aboard NEFSC bottom trawl surveys in the summer flounder seasonal survey strata were used to estimate weighted diet composition over time (see [methods](#sec:methodsdiet)), as well as length composition of sampled fish and sample size. Here we show annual and seasonal weighted diet composition for major prey (averaging over 10 percent of diet across years). Food habits data for summer flounder have been collected from 1977-present [@smith_trophic_2010]. 

Using the criteria above, more of the spring diet composition is captured than the fall or winter, suggesting more diverse diets in those seasons. Diet compositions across seasons and years include "*Loligo*" (recently renamed *Doryteuthis*) squid species as a major component, as well as unidentified bony fish ("other fish" and "osteichthies" below). Cephalopods in addition to *Loligo* are major components in winter and spring, but not fall. Scombrids, *Scomber scombrus* (Atlantic mackerel),  *Clupea hargengus* (Atlantic herring), and *Stenotomus chrysops* (scup) are intermittent spring diet components, while *Etrumeus teres* (round herring), Gadids and *Urophycis regia* (spotted hake) appear in fall diets. *Ammodytes* sp. (sandlance) appear in both spring and fall diets, but more consistently in fall.

While trends are difficult to discern in diet components, Gadids only appear in fall diets in 2008 and may be increasing, while spotted hake proportions may be decreasing over the same period. Similarly, the proportion of scup in spring diets may be decreasing since 2010. Full seasonal diet compositions with numbers of stomachs and survey stations are part of the diet dataset for analysis but not visualized in this summary.
