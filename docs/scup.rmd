...
bibliography: C:/Users/sean.hardison/Documents/git/ecsa/docs/ECSA_bibliography.bib
...
An optimal interpolation procedure was used to estimate NE Shelf surface and bottom temperatures for two seasonal time frames (see [methods](#methodstempsalin)). The temperature estimates were standardized to April 3 and October 11 for spring and fall over the period 1968-2018. Surface and bottom temperature within the scup stock areas are shown below.  The fall warming trends magnitude is greater than in the spring. As scup have been shown to be found in greatest abundance at temperatures from 13 C to 16 C [@Fritz65], warming waters in the region may provide better habitat.  
...
fall_min <- min(fall_surface_temp$df$Time)
...
Before 2002, scup biomass was estimated to be the highest in 1978 with an apparently low period in the 1990s. The period from 2000 to 2010 was characterized by almost annual shifts in biomass from high to low to high until 2007 when there was a three year increase.  After 2010, biomass didn't fall below 6.5 and four years were above 7 whereas only three years prior to 2010 were above 7. 
...
if (nrow(spring_sae) != 0){
  ...
  if (interactive){
    ...
    #                       width = 0.9, height_svg = 5),opts_hover(css = "r:2pt;") )
        ...
        girafe_options(girafe(code = print(title + biomass_plt + plot_layout(nrow = 2)),
...
```{r, doc-correction, fig.align = "center", echo = F, fig.cap = "Spring (a) and fall (b) linear slope coefficients used to date correct surface temperature to standard spring and fall dates; same for spring (c) and fall (d) bottom temperature correction.", out.width="50%"}
...
### {#fig:oi-mae}
```{r, oi-mae, fig.align = "center", echo = F, fig.cap = "Mean absolute error by 0.1 degree latitude and longitude intervals for spring (a) and fall (b) surface temperature; same data for spring (c) and fall (d) bottom temperature. Red indicates a positive error where blue indicates negative.", out.width="50%"}
par(mar=c(0,0,0,0), xpd=NA, mgp=c(0,0,0), oma=c(0,0,0,0), ann=F, mfrow = c(2,2))
