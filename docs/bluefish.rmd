...
```{r strata-map, eval = T, echo = FALSE, fig.cap= paste0("Strata map for bluefish (",ifelse(knitr::is_latex_output(), '\\textit{Pomatomus saltatrix}','*Pomatomus saltatrix*'),") on the NE shelf"), message = FALSE, warning = FALSE, fig.align='center'}
...
```{r ecosystem-hab-area, fig.height=4, eval = T}
threshold <- 0.5
...
```{r spring-sae-plt, fig.height = adjust_for_facet, eval = print_plot[1]}
if (nrow(spring_sae) != 0){
  if (do_transform){
...
```{r fall-sae-plt, fig.height = adjust_for_facet, eval = print_plot[2]}
if (nrow(fall_sae) != 0){
  if (do_transform){
...
knitr::kable(
  df, booktabs = TRUE, col.names = c("Variable name","Taxa"), format = "markdown",
...
df<-df[-c(1) ,c("Variables","Notes","References")]
  ...
  knitr::kable(
    df,
    format = ifelse(knitr::is_latex_output(),"latex","markdown"),
    booktabs = T,
