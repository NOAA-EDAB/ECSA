...
bibliography: ["`r here::here('ECSA_bibliography.bib')`"]
link-citations: true
...
```{r strata-map, eval = T, echo = FALSE, fig.cap= "Strata map for summer flounder (*Paralichthys dentatus*) on the NE shelf", message = FALSE, warning = FALSE, fig.align='center'}
...
```{r spring_sae_plt, fig.height = 6, eval = F}
...
```{r fall_sae_plt, fig.height = 6, eval = F}
...
1. `create_template.R` is run to create the species specific `.Rmd` document with the following arguments (for additional arguments, refer to `?create_template`). For summer flounder:
``` {r, cre_tmp, echo=T, eval =F}
create_template(stock_name = summer-flounder, render_book=TRUE, output_dir = here::here("docs"))
```
...
3. This document (the one you are viewing) will then automatically open in a web browser and all associated files can be found in the `docs\summer-flounder` folder in your current working directory.
