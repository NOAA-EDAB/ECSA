...
Food habits collected aboard NEFSC bottom trawl surveys in the {{COMMON_NAME}} seasonal survey strata were used to estimate weighted diet composition over time (see [methods](#sec:methodsdiet)), as well as length composition of sampled fish and sample size. Depending on sample size (number of survey tows and stomachs) seasonal/annual estimates of diet composition or temporally aggregated estimates (5 or 10 year blocks) can be reported. {{COMMON_NAME}} have been collected as a "secondary" priority species from 1977-1980 and from 1985-present (Smith and Link, 2010). 

```{r dietdat}
#{{COMMON_NAME}} stomachs are collected in small (<30 cm), medium (31-70 cm), and large (>70 cm) size strata. Over 4,500 {{COMMON_NAME}} stomachs have been collected to date from over 1,400 survey tows. Predator length ranged in size from 3 to 118 cm.
#[add: number+hauls sampled and length comp of sampled {{COMMON_NAME}} in each time block below]
```


### Figures
```{r diet, eval=T, echo = FALSE, fig.align='center'}
...
diet <- df_diet %>%
  dplyr::select(year, season, prey, relmsw)
  
dietmost <- diet %>%
    dplyr::group_by(season, prey) %>%
    dplyr::filter(max(relmsw)>1.0)
  
dietmain <- diet %>%
    dplyr::group_by(season, prey) %>%
    dplyr::filter(mean(relmsw)>5.0)  
...
compplot + facet_wrap("season") #+ theme(legend.position="none")

```

### Data
```{r diet data, echo = F}
#datatable(df_diet, rownames = FALSE)
```
...
The habitat models were evaluated for fit based on out-of-bag classification accuracy. For occupancy models accuracy, AUC (Area Under the ROC Curve), and Cohen’s Kappa were calculated using the irr R package [@irr, version 0.84]. For regression models, the variance explained by the model, mean absolute error, the root mean square error, and bias were calculated using the Metrics R package [@metrics, version 0.1.3]. To evaluate variable importance in both occupancy and regression models, we plotted the number of times a variable was the root variable versus the mean minimum node depth for the variable, highlighting the top ten important variables [@randomForestExplainer R package, version 0.9]. For occupancy models we also plotted the Gini index decrease versus accuracy decrease, whereas for the regression models we plotted node purity increase versus MSE increase, also highlighting the top ten most important variables. 
