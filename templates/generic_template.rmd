...
spring_stock_area_plt <- ggplot(data = spring_stock_area_df,
         aes(x = Time, 
            y = Value, color = Var, group = Var,
                 tooltip = tooltip,
                 data_id = Time)) +
  geom_line()+
  {if(interactive) geom_point_interactive(size = 0)
    else geom_line(data = spring_stock_area_df[spring_stock_area_df$Var == "Series",],aes(color = Var))} +
    scale_color_manual(values = 
                         c("Regime mean" = "#b2df8a",
                         "Trend" = "#a6cee3",
                         "Series" = "#1f78b4"))+
...
fall_stock_area_plt <- ggplot(data = fall_stock_area_df,
         aes(x = Time, 
            y = Value, color = Var, group = Var,
                 tooltip = tooltip,
                 data_id = Time)) +
  geom_line()+
  {if(interactive) geom_point_interactive(size = 0)
    else geom_line(data = fall_stock_area_df[fall_stock_area_df$Var == "Series",],aes(color = Var))} +
    scale_color_manual(values = 
                         c("Regime mean" = "#b2df8a",
                         "Trend" = "#a6cee3",
                         "Series" = "#1f78b4"))+
...
```{r ecosystem_hab_area_fall}
ecosystem_hab_area_df <- 
  ecosystem_habitat_area %>% 
    dplyr::filter(threshold == !!threshold) %>% 
    dplyr::select(season, Time = year, Value = value, area) 

if (any(stringr::str_detect(unique(ecosystem_hab_area_df$season), "fall"))){
  
subareas <- ecosystem_hab_area_df %>% 
  filter(season == "fall") %>% 
  pull(area) %>% 
  unique()
...
for (i in subareas){
  ...
  assign('fall_output', rbind(fall_ecosystem_area, fall_output))
...
```{r ecosystem_hab_area_spring}

if (any(stringr::str_detect(unique(ecosystem_hab_area_df$season), "spring"))){
...
for (i in subareas){
  ...
  assign('spring_output', rbind(spring_ecosystem_area, spring_output))
...
```{r spring-ecosystem-area-plt, fig.height=4, eval = print_plot[1]}
...
```{r fall-ecosystem-area-plt, fig.height=4, eval = print_plot[2]}
...
fall_ecosystem_area_plt <- 
  ggplot(data = fall_ecosystem_area_df, 
         aes(x = Time, 
          y = Value, group = Var)) +
   geom_line(aes(color = Var)) +
   {if(interactive) geom_point_interactive(aes(tooltip = tooltip, data_id = Time,
