...
spring_sae <- readr::read_csv(here::here("data/stock_data/spring_sae_tb_sum.csv"))
fall_sae <- readr::read_csv(here::here("data/stock_data/fall_sae_tb_sum.csv"))
...
spring_sae_biomass <- 
  ...
  dplyr::filter(svspp == svspp_) %>% 
  dplyr::select(-svspp) %>%
  tidyr::unite(., "stock_treatment",stocks, treatment, biomass, sep = "_") %>%
  tidyr::gather(.,Var, Value, -year,-stock_treatment) %>% 
  tidyr::separate(., stock_treatment, into = c("area","treatment","biomass"), sep = "_") %>% 
  dplyr::rename(Time = year) %>% 
  dplyr::filter(biomass == "yes") %>% 
  mutate(treatment = as.factor(treatment))

spring_sae_bio_nzero <- spring_sae_biomass %>% 
  dplyr::filter(stringr::str_detect(Var, "ztotal"))

spring_sae_bio_nzero_log <- spring_sae_biomass %>% 
  dplyr::filter(stringr::str_detect(Var, "zlog"))

spring_sae_bio <- spring_sae_biomass %>% 
  dplyr::filter(Var == "total")

spring_sae_bio_log <- spring_sae_biomass %>% 
  dplyr::filter(Var == "log10total")

spring_sae_bio_nzero_plt <- 
  ggplot(spring_sae_bio_nzero) +
  geom_line(aes(x = Time, y = Value, color = treatment)) +
  facet_grid(area ~ .) +
  guides(color = guide_legend(title = ""))

spring_sae_bio_nzero_log_plt <- 
  ggplot(spring_sae_bio_nzero_log) +
  geom_line(aes(x = Time, y = Value, color = treatment)) +
  facet_grid(area ~ .) +
  guides(color = guide_legend(title = ""))

spring_sae_bio_plt <- 
  ggplot(spring_sae_bio) +
  geom_line(aes(x = Time, y = Value, color = treatment)) +
  facet_grid(area ~ .)+
  guides(color = guide_legend(title = ""))

spring_sae_bio_log_plt <- 
  ggplot(spring_sae_bio_log) +
  geom_line(aes(x = Time, y = Value, color = treatment)) +
  facet_grid(area ~ .)+
  guides(color = guide_legend(title = ""))

# spring_sae_bio_plt + spring_sae_bio_log_plt +
# spring_sae_bio_nzero_plt + spring_sae_bio_nzero_log_plt +
#   plot_layout(ncol = 2)
  

# spring_plots <- lapply(X = c(spring_sae_bio_plt,
#                    spring_sae_bio_log_plt,
#                    spring_sae_bio_nzero_plt,
#                    spring_sae_bio_nzero_log_plt),
#                    FUN = function(x){
#                      plotly::ggplotly()
#                    })
plt <- subplot(ggplotly(spring_sae_bio_log_plt),
        ggplotly(spring_sae_bio_plt),
        ggplotly(spring_sae_bio_nzero_log_plt),
        ggplotly(spring_sae_bio_nzero_plt),
        shareX = F, nrows = 2,margin = 0.05) %>% 
  layout(title = "")

nlines <- 12 * length(treatments)

for (i in 1:nlines){
  if (i > (nlines/4)){
    plt$x$data[[i]]$showlegend <- F
  }  
}
