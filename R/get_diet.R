...
get_diet <- function(overwrite = FALSE) {
  ...
  # #CHOOSE byvars and byvar names (e.g. geoarea, sizecat, season, year, etc.).  Order according to output and fill as necessary or leave 'x'. Allows for max of 4.
  # #1
  #allsum$byvar1=allsum$year
  name_byvar1=c('year')
  # 
  # #2
  #allsum$byvar2=allsum$season
  name_byvar2=c('season')
  #allsum$byvar2='x'
  #name_byvar2=c('x')
  # 
  ...
  #changed this dont want to write over allsum which is what is being read in again below, just keep it
  # allsum=allsumfin
  # 
  ...
  if(unique(stomstats$byvar1=='x')){stomstats$byvar1=NULL} 
  if(unique(stomstats$byvar2=='x')){stomstats$byvar2=NULL}
  if(unique(stomstats$byvar3=='x')){stomstats$byvar3=NULL}
  if(unique(stomstats$byvar4=='x')){stomstats$byvar4=NULL}
  
  names(stomstats)[names(stomstats)=='byvar1']=name_byvar1                                  
  names(stomstats)[names(stomstats)=='byvar2']=name_byvar2
  names(stomstats)[names(stomstats)=='byvar3']=name_byvar3                                  
  names(stomstats)[names(stomstats)=='byvar4']=name_byvar4                                  
  
  #stomstats with nstom, saving svspp, year, season, nstom FIX THIS SO IT ISNT HARDCODED
  nstom=stomstats[,c(1,2,3,8)]
  
  
  ###################Begin allwt code###################################
  
  #read in allfh again for weighted diet props
  #no, use allsum already in memory that is all summer flounder data
  #load("allfhsg.RData")
  
  #change this later to a set of sensible names, for convenience just go with it
  #allfh
  allfh=allsum
  ...
  final2=merge(final, nstom, by=c('svspp', 'year', 'season'), all.x=T )
